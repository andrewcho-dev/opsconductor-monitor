...
from backend.api.routes import alerts, addons, system, targets, users
from backend.api.socketio_manager import sio, socket_app, setup_alert_broadcasts
from backend.api.auth import decode_token
from backend.core.addon_registry import get_registry
from backend.core.trap_receiver import start_receiver, stop_receiver
...
async def redis_alert_subscriber():
    ...
    import redis
    import json
    from backend.api.socketio_manager import broadcast_event
...
async def webhook_handler(path: str, request: Request):
    ...
    """
    from backend.core.webhook_receiver import handle_webhook
